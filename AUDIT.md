...
- `odoo/apps/<module>.html` for every app in the catalogue, generated by `build.py` from `_src/apps_content.json`
  (pulled from odoo.com/app/<slug> by `python _src/fetch_odoo.py`): headline + lead, up to six product sections,
  the official YouTube video behind a click-to-play facade (youtube-nocookie), a swipeable strip of odoocdn
  screenshots, “how TechNext implements it”, related apps and the Odoo S.A. attribution line. Payroll has no
  odoo.com page and falls back to our own copy. External images hide themselves on error.
